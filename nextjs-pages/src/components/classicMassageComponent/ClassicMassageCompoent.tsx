  ...
  return (
    <>
          ...
          return (
            <div key={index} className={styles.classic_massage}>
              <div className={styles.classic_massage__title}>
                <h3>
                  {el.title} <span>{el.titleColor}</span>
                </h3>
                <p>{el.description}</p>
              </div>
      ...
      <div className={styles.classic_info}>
        <p className={styles.classic_info__description}>
          JEŚLI NIE WIESZ JAKI MASAŻ JEST DLA CIEBIE - ZADZWOŃ !{" "}
        </p>
        <div className={styles.classic_info__phone}>
          <FaPhoneAlt fontSize={36} /> <a href="tel:+48530907750">530907750</a>
        </div>
      </div>
    </>
  );
